...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'contributor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<LibraryInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/libraries` : null),
    () => getLibraries({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Library',
      path: isTenantUser ? `/libraries/view/${data?.[0]?.id}` : '/libraries',
      entity: 'library',
      service: AccessServiceEnum.PROJECT,
      icon: FiBook,
    },
    { name: 'Mp 3', path: '/mp-3s', entity: 'mp3', service: AccessServiceEnum.PROJECT, icon: FiMusic },
